...
public class CustomJwtFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        // 클라이언트가 요청하면 토큰을 꺼내서 토큰의 유효성을 체크하고 SecurityContextHolder 안에 토큰을 저장
        HttpServletRequest httpServletRequest = (HttpServletRequest) request;
        String jwt = getToken(httpServletRequest);
        if (StringUtils.hasText(jwt) && tokenProvider.validatorToken(jwt)) {
            // 유효성 체크가 완료되면 토큰을 이용해서 인증정보를 꺼내기
            Authentication authentication = tokenProvider.getAuthentication(jwt);
            SecurityContextHolder.getContext().setAuthentication(authentication);
            System.out.println("인증된 사용자 입니다. 인증 정보를 저장합니다.");
        } else {
    ...
    // 클라이언트의 요청 정보에서 토큰을 꺼내서 리턴하는 메소드

...
public class FactoryExample {
	...
	public void getResult(Factory mainFactory, char mainFactorySkill,
						  IWorkingTogether partner) {
		int coworker = 0;
		String CoName = "";
		if (partner instanceof CarFactory) {
			coworker = ((CarFactory) partner).makeProducts('B');
			CoName = ((CarFactory) partner).getFactoryName();
		} else {
			coworker = ((TVFactory) partner).makeProducts('C');
			CoName = ((TVFactory) partner).getFactoryName();
		}
		...
		System.out.println("2. 파트너 공장 [" + CoName + "] = " + coworker);
		// TODO
		System.out.println("--------------------------------------------------------------");
	}
}
...
public class CoffeShop {
	public static void main(String[] args) {
		...
		beverage[0] = new Coffee1("Cappuccino");
		beverage[1] = new Coffee1("CafeLatte");
		beverage[2] = new Tea("ginsengTea");
		beverage[3] = new Coffee1("CafeLatte");
		beverage[4] = new Tea("redginsengTea");
		getSalesInfo(beverage);
		System.out.println("총 판매 금액==>"+getTotalPrice(beverage));
		System.out.println("Coffe의 판매 개수=>"+Coffee1.amount+"잔");
	...
	private static void getSalesInfo(Beverage[] beverage) {
		int sellCount = 0;
		for (Beverage sell : beverage) {
			sellCount += 1;
			System.out.println(sellCount + "번째 판매 음료는 " + sell.getName() + "이며, 가격은 " + sell.getPrice());
		}
	}
